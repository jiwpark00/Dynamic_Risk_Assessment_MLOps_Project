...
if config['output_model_path'] == 'practicemodels':
    prediction_model = pickle.load(
        open(model_path + '/trainedmodel.pkl', 'rb'))
else:
    prediction_model = pickle.load(
        open(
            model_path +
...
def predict():
    ...
    # if dataset_location is not passed, this API call doesn't error - it
...
def scoring_stats():
    ...
    return {"f1 output is:": str(f1_output)}
...
def summary_stats():
    ...
    return {"Here is the summary stats": summarystats}
...
def diagnostics_stats():
    ...
    return {
        'NA percent is (from each numeric col in the order)': na_p,
        'execution time is (0 = training, 1 = ingestion): ': execute_times,
