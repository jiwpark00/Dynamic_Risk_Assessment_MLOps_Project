...
# excluding first column and last column that's actually a target
numeric_cols = dataset_for_stats.columns[1:-1]
...
def check_NA():
    ...
    for col in numeric_cols:
        ...
        napercents = [nas[i] / len(dataset_for_stats.index)
...
def outdated_packages_list():
    ...
    requirements_installed = pd.merge(
        requirements,
        installed_df,
        how='inner',
        left_on=['package_name'],
    ...
    requirements_installed_recent = pd.merge(
    ...
    if config['output_model_path'] == 'practicemodels':
        output_path_dependencies = os.getcwd() + '/' + \
            os.path.join(config['prod_deployment_path']) + '/dependencies_status.csv'
        requirements_installed_recent.to_csv(
            output_path_dependencies, index=False)
    else:
        output_path_dependencies = os.getcwd() + '/' + os.path.join(
            config['prod_deployment_path']) + '/dependencies_status_' + todays_date + '.csv'
        requirements_installed_recent.to_csv(
            output_path_dependencies, index=False)
