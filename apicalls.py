...
def run_api():
    ...
    response1 = subprocess.run(
        [
            ...
            "POST",
            URL +
            ':8000/prediction?dataset_location=testdata/testdata.csv'],
        capture_output=True).stdout  # put an API call here
    response1 = response1.decode("utf-8")
    response2 = requests.get(
        URL + ':8000/scoring').content  # put an API call here
    response2 = response2.decode("utf-8")
    response3 = requests.get(
        URL + ':8000/summarystats').content  # put an API call here
    response3 = response3.decode("utf-8")
    response4 = requests.get(
        URL + ':8000/diagnostics').content  # put an API call here
    response4 = response4.decode("utf-8")

    # combine all API responses
    responses = response1 + '\n' + response2 + \
        response3 + response4  # combine reponses here
    ...
    if config['output_model_path'] == 'practicemodels':
        ...
        dataset_csv_path = os.path.join(config['output_model_path'])
        output_path = os.getcwd() + '/' + dataset_csv_path + '/apireturns2.txt'
